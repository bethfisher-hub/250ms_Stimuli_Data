...
trial_vars<- c( "participant",
                ...
                "similarity", "response_time", "catchnumber", "Ecc",
                "catchnumberprac", "catchresponse", "Catch", "screen_size_x",
                "screen_size_y","viewerdistancecm", 'viewer_distance',"trialnumber","Ecc", "response_time_catch")
...
dfcatch <- data[which(data$Catch == 1),]

# Check Catch Trial scores

catch_trial_checker <- function(dfcatch){
  
  subjectlist <- sort(unique(dfcatch$participant))
  print("Catch scores")
  for (participant in subjectlist){
    subjectdf <- dfcatch[which(dfcatch$participant == participant),] 
    catch_num = nrow(subjectdf)
    catch_correct = nrow(subset(subjectdf, catchnumber == catchresponse))
    
    print(paste("Subject",participant,":",catch_correct,"/",catch_num))
  }
}

catch_trial_checker(dfcatch)

# Catch trial response time

rt_avg_catch <- function(dfcatch){
  return(mean(dfcatch$response_time_catch))
}

rt_avg_check_catch <- function(dfcatch){
  
  subjectlist <- sort(unique(dfcatch$participant))
  print("RT avg")
  for (participant in subjectlist){
    subjectdf <- dfcatch[which(dfcatch$participant == participant),] 
    rt = rt_avg_catch(subjectdf)
    print(paste("Subject:",participant,"mean rt",rt))
  }
}

rt_avg_check_catch(dfcatch)
...
screen_size <- function(data){
  
  data <- subset(data, !is.na(screen_size_x), !is.na(screen_size_y))

  width <- as.numeric(substr(as.character(data$screen_size_x)[1],1,6))
  height <- as.numeric(substr(as.character(data$screen_size_y)[1],1,6))
...
screen_parameters <- function(data,individual=FALSE){
  
  subjectlist <- sort(unique(data$participant))
  print("Screen Parameters")
  screen_fail = 0
  viewing_fail = 0
  for (participant in subjectlist){

    subjectdf <- data[which(data$participant == participant),] 
...
# Remove participants 1, 7 and 9
dftrials <- dftrials[which(dftrials$ID != 1 & dftrials$ID !=7 & dftrials$ID !=9),]


# Check average Response Time
rt_cutoff = 700 # mean reaction times must be above this

rt_avg <- function(dftrials){
  return(mean(dftrials$response_time))
}
...
pass_compare_list_Fisher <- function(dftrials){
  ...
  for (ID in subjectlist){ # go through subject by subject
    ...
    correlation_list <- c(correlation_list, FisherZ((matrixcor_spear(subjectdf))))
...
pass_compare_list_Fisher(dftrials)
...
pass_compare_list_plot <- function(dftrials){
  ...
  for (ID in subjectlist){ # go through subject by subject
    subjectdf <-  dftrials[which(dftrials$ID == ID),]  # select the ID for subject of interest
    correlation_list <- c(correlation_list, (matrixcor_spear(subjectdf)))
    
    plot <- plot(correlation_list, main = '1st and 2nd pass correlation - rho',
                 xlab='Participant',ylab='r',xlim=c(1,12),pch = 21, col="black", cex=1.5)
    axis <- axis(1,seq(1,14,1))
  }
  return(correlation_list)
  return(plot)
...
t.test(corrlist, corlist1)

wilcox.test(corrlist, corlist1)
...
# Keep participant for other things?
...
matdfall <- rbind(matdf2, matdf3, matdf4, matdf5, matdf6, matdf8, matdf11, matdf12, matdf13, matdf14, matdf15)
